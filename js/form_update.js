...
async function submitForm() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            window.location.href = 'uploaddocshistory.php';
        } else {
        ...
        requiredFields.forEach(field => {
            ...
            if (input && !input.disabled) {
                ...
                if (!input.value.trim() && !field.optional) {
                    ...
                    const studentIdPattern = /^[0-9]{4}-[0-9]{5}$/; // 4 digits, hyphen, 5 digits

                    if (studentId === '') {
                        isValid = false;
                        errors.push('Student ID is required');
                        input.classList.add('is-invalid');
                        if (feedback) feedback.textContent = 'Student ID is required';
                    }
                    else if (studentId.toUpperCase() !== 'N/A' && !studentIdPattern.test(studentId)) {
                        // Only run pattern check if NOT "N/A"
                        isValid = false;
                        errors.push('Student ID must be in format: 1234-56789 or "N/A"');
                        input.classList.add('is-invalid');
                        if (feedback) feedback.textContent = 'Format must be: 1234-56789 or "N/A"';
                    }
