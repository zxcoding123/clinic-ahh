...
if ($child_id) {
    ...
    $sql_child = "
        ...
        FROM children_history c
        LEFT JOIN patients_history p ON c.parent_id = p.id
        WHERE c.id = ?";
    ...
    if (!$child_data) {
        ...
        WHERE patient_id = (SELECT user_id FROM patients_history WHERE id = ?)";
    ...
    <div class="section-container">
        ...
        <div class="section-content">
            ...
            if (!empty($hospital_admissions)) {
                ...
                // If no admissions, show at least one "No" row
                $hospital_admissions_formatted[] = ['year' => '', 'reason' => ''];
            }
            ?>

            <table>
                <?php foreach ($hospital_admissions_formatted as $admission):
                    $has_data = !empty($admission['year']);
                ?>
                    <tr>
                        <td style="width: 10%;">
                            <span class="checkbox-view <?php echo !$has_data ? 'checked' : ''; ?>">✓</span> No
                        </td>
                        <td style="width: 10%;">
                            <span class="checkbox-view <?php echo $has_data ? 'checked' : ''; ?>">✓</span> Yes
                        </td>
                        <td style="width: 30%;">
                            Year: <span class="view-only-text"><?php echo htmlspecialchars($admission['year']); ?></span>
                        </td>
                        <td style="width: 50%;">
                            Reason/s: <span class="view-only-text"><?php echo htmlspecialchars($admission['reason']); ?></span>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </table>
