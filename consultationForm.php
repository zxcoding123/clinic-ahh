...
if ($appointment_id > 0 && $patient_id > 0) {
    ...
    if (mysqli_num_rows($checkResult) > 0) {
        ...
        $checkUserSql = "SELECT user_id FROM patients WHERE id = $patient_id LIMIT 1";
...
function generateConsultationPDF($appointment, $consultationData)
{
    ...
    $pdf->SetSubject('Medical Consultation');
    ...
    $pdf->Cell(0, 10, 'Complaints (Signs and Symptoms):', 0, 1);
    ...
    // Title
    $pdf->SetFont('helvetica', 'B', 16);
    $pdf->Cell(0, 10, 'MEDICAL CONSULTATION FORM', 0, 1, 'C');
    $pdf->Ln(5);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['form_type']) && $_POST['form_type'] === 'consultation') {
    ...
    $consultation_id = isset($_GET['appointment_id']) ? intval($_GET['appointment_id']) : 0;

    if ($consultation_id > 0) {
        // Fetch the row based on consultation_id
        $sql = "SELECT id FROM consultations WHERE appointment_id = $consultation_id LIMIT 1";
        $result = mysqli_query($conn, $sql);

        if ($row = mysqli_fetch_assoc($result)) {
            $consultationDbId = $row['id'];  // <- This is the real consultation table ID
            echo $consultationDbId;
        } else {
        }
    }
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        ...
        if (!$patient) {
            ...
            $signature_data = trim($_POST['physician_signature']);
...
INSERT INTO medical_consultations (
    ...
    temperature, heart_rate, respiratory_rate, oxygen_saturation, complaints, diagnosis,
    treatment, test_results, staff_name, staff_signature
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                mysqli_stmt_bind_param(
                    $stmt,
                    "iddsssssssssss", // 13 characters for 13 params
                    $consultationDbId, // int
                    $height,           // double
                    $weight,           // double
                    $blood_pressure,   // string
                    $temperature,      // string or double
                    $heart_rate,       // string or int
                    $oxygen_saturation, // string or int
                    $respiratory_rate, // string or int
                    $complaints,       // string
                    $diagnosis,        // string
                    $treatment,        // string
                    $test_results,     // string
...
WHERE a.id = ?";
                ...
                $sql = "UPDATE consultations SET status = 'Completed' WHERE appointment_id = ?";
                $stmt = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    throw new Exception('Failed to prepare appointment update query: ' . mysqli_error($conn));
                }
             ...
             WHERE a.id = ?";
                ...
                if ($patientEmail) {
                    ...
                    $subject = "WMSU Health Services - Medical Consultation Summary";
                    $body = "
        <h2>WMSU Health Services - Medical Consultation Summary</h2>
        <p>Dear " . htmlspecialchars($appointment['first_name']) . ",</p>
        <p>Your medical consultation on " . date('F j, Y') . " has been completed. Please find attached your consultation summary and prescription (if applicable).</p>
    ...
    ";
        ...
        ");
                ...
                $notificationTitle = "Medical Consultation Form Issued!";
                $notificationDesc = "Your medical consultation and certificate has been issued and sent to your email!";
                ...
                // Redirect to medical-appointments.php
                header("Location: medical-appointments.php");
                exit();
            } catch (Exception $e) {
...
$sql = "
  ...
  CONCAT(
    ...
    WHERE a.id = ? AND a.appointment_type = 'medical' AND a.status = 'Pending'";
$stmt = mysqli_stmt_init($conn);
if (mysqli_stmt_prepare($stmt, $sql)) {
    mysqli_stmt_bind_param($stmt, "i", $appointment_id);
    mysqli_stmt_execute($stmt);
    ...
    <div id="app" class="d-flex">
        ...
        <div class="main-content w-100">
            ...
            <div class="print-container" id="consultation-container">
                ...
                <h2 class="text-center mb-4">Medical Consultation Form</h2>
                <hr>
                <form id="consultationForm" method="POST">
                    ...
                    <input type="hidden" name="consultation_type" value="medical">
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-md-3">
                            <label class="fw-bold" for="weight">Height (Cm):</label>
                            <input id="weight" type="text" class="form-control" name="height" value="<?php echo htmlspecialchars(isset($_SESSION['consultation_data']['weight']) ? $_SESSION['consultation_data']['weight'] : ''); ?>">
                        </div>
                        <div class="col-md-3">
                    ...
                    <div class="row mb-2">
                        <div class="col-md-3">
                            <label class="fw-bold" for="temperature">Respiratory Rate:</label>
                            <input id="temperature" type="text" class="form-control" name="respiratory_rate" value="<?php echo htmlspecialchars(isset($_SESSION['consultation_data']['temperature']) ? $_SESSION['consultation_data']['temperature'] : ''); ?>">
                        </div>
                        <div class="col-md-3">
                    ...
                    <div class="mb-2">
                        <label class="fw-bold" for="complaints">Complaints (Signs and Symptoms):</label>
                        <textarea id="complaints" class="form-control" name="complaints" rows="3">This patient is here for: <?php echo htmlspecialchars(isset($_SESSION['consultation_data']['complaints']) ? $_SESSION['consultation_data']['complaints'] : $appointment['reason']); ?></textarea>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            <label class="fw-bold" for="treatment">Treatment (Management):</label>
                        ...
                        <div class="col-md">
                            <br> <br>
                            <label class="fw-bold" for="test_results">Test Results:</label>
                            <textarea id="test_results" class="form-control" name="test_results" rows="5"></textarea>
                        </div>
                    </div>
                    <br> <br>
                    <div class="mt-3">
            ...
            <div class="print-container prescription-container" id="prescription-container" style="display: none;">
                ...
                <div class="prescription-content">
                    <form id="prescriptionForm">
                        ...
                        <input type="hidden" name="physician_signature" id="signature-data-physician" value="<?php echo htmlspecialchars(isset($_SESSION['consultation_data']['physician_signature']) ? $_SESSION['consultation_data']['physician_signature'] : ''); ?>">
                        <br>
